                            ...
                            const std::vector<std::pair<std::string, cv::Vec3f>> &centers)
{
    ...
    if (!ofs.is_open())
    {
        SENDLOGF_TAG(LOG_TAG, "[IntensityOnly] failed to save centers JSON: %s", path.c_str());
        return false;
    }
    ofs << j.dump(2);
    SENDLOGF_TAG(LOG_TAG, "[IntensityOnly] saved %s (N=%zu)", path.c_str(), centers.size());
    return true;
}
                             ...
                             float alpha)
{
    ...
    auto runOneROI = [&](const std::vector<CartesianPointRGB> &roiPts,
                         const cv::Vec3f &Cprev) -> std::optional<cv::Vec3f>
    {
    ...
    else
    {
        ...
        auto C = runOneROI(points, /*Cprev=*/cv::Vec3f(0, 0, 0));
        if (C.has_value())
    ...
    return saveCentersJSON(regDir + "/targets3d_adjust.json", centersOut);
}
